...
`config/cities.example.jsonc`): no agency publishes a GTFS feed, so line
...
- `cities.example.json` — the real city entries for Ouagadougou,
  Koudougou, and Bobo-Dioulasso (plus the `burkina-faso` country entry).
  Merged into your own `config/cities.json` by `make use-example`.
...
- `cities/<city>/` — the app-ready JSON `gtfs_routes_to_json.py` /
  ...
  would normally live at `data/cities/<city>/`). `dedougou` and
  `ouahigouya` only have this generated output, not a matching entry in
  `cities.example.json` - leftover from an earlier, wider deployment; still
  useful as fixture data, just without a registry entry to copy.

## Using it

Easiest: `make use-example COUNTRY=burkina-faso` from the repo root - merges
all three cities' entries into `config/cities.json` and copies their data
into `data/`, ready for `make dev`. Add `CITY=ouagadougou` (or any single
slug) to bring in just one.

To do it by hand instead: copy the relevant entry from `cities.example.json`
into `config/cities.json`, then either copy `gtfs/<city>/` to
